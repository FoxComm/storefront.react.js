...
function getLoader(opts, plugins) {
  const { root = '/' } = opts;
  return typeof opts.Loader === 'function' ? new opts.Loader(root, plugins) : new FileSystemLoader(root, plugins);
}
...
module.exports = postcss.plugin(PLUGIN_NAME, (opts = {}) => {
  ...
  return (css, result) => {
    ...
    return postcss([parser.plugin]).process(css, { from: inputFile }).then(() => {
